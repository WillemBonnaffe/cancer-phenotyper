...
def parse_args():
    ...
    parser.add_argument('--CSV_PATH', type=str, help='Optional path to CSV file with WSI paths')
    parser.add_argument('--CSV_COLUMN', type=str, help='Column name in CSV containing WSI paths')
...
## Get paths from CSV and otherwise from folder
if args.CSV_PATH and args.CSV_COLUMN:
    import pandas as pd
    df = pd.read_csv(args.CSV_PATH)
    if args.CSV_COLUMN in df.columns:
        WSI_PATHS = df[args.CSV_COLUMN].dropna().tolist()
    else:
        raise ValueError(f"Column '{args.CSV_COLUMN}' not found in CSV.")
else:
