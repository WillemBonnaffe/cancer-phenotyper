...
def grouped_stratified_kfold_split_(labels, groups, num_folds, send_to_test_th=3):
    """
    """
    # print("grouped stratified k-folds")
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs, device="cpu", patience=90):
    """
    """
...
def load_train_model(model, train_dataset, val_dataset, criterion, optimizer, batch_size, num_epochs, device, pt_output, model_name, plot_diagnostic):
    """
    """
    ## Dataloaders
    ...
    ## Train the model
    t0 = time.time()
    model, train_loss, val_loss = train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs, device)
    tf = time.time()
    print(f"trained model in {tf-t0:.2f}s")
    plot_loss(train_loss, val_loss, pt_output + "fig_" + str(model_name) + "_loss.png")

    ## Evaluate
    outputs, _, _, true = evaluate_model(model, train_loader, device)
    plot_roc(true, outputs, pt_output + "fig_" + str(model_name) + "_accuracy_train.png")
    plot_predictions(true, outputs, pt_output + "fig_" + str(model_name) + "_predictions_train.png")
    outputs, _, _, true = evaluate_model(model, val_loader, device)
    plot_roc(true, outputs, pt_output + "fig_" + str(model_name) + "_accuracy_val.png")
    plot_predictions(true, outputs, pt_output + "fig_" + str(model_name) + "_predictions_val.png")
...
def plot_roc_(y_true, y_scores, pt_save):
    num_labels = y_scores.shape[1]
