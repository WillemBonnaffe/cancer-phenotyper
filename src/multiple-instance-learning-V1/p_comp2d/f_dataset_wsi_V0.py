...
class WSIDatasetByInstance:
    ...
    def __init__(self, wsi_path_list, tile_coordinate_list, labels,
        ...
        self.augment = augment  # New property to control augmentation

        ## Initialize lists to store instance data
        self.loaded_instance_coordinates = []  # Store instance coordinates
        self.loaded_labels = []  # Store instance labels
        self.loaded_wsi_paths = []  # Store WSI paths
        self.loaded_sample_ids = []  # Store sample IDs
    ...
    def __getitem__(self, index):
        ...
        """
        ## Check if tiles should be preloaded
        if self.preload_tiles:
            instance = self.loaded_instances[index]
        else:
            ## Load the tile on demand
            tile_coordinates = [self.loaded_instance_coordinates[index]]
            wsi_path = self.loaded_wsi_paths[index]
            instance = slide_path_to_tiles_at_coordinates(wsi_path, tile_coordinates, self.tile_size, self.level)[0]
