...
parser.add_argument('--save_checkpoints', action='store_true', help='Save/load individual embeddings from checkpoint folder')
...
## Create checkpoint folder if requested
if args.save_checkpoints:
    checkpoint_folder = os.path.join(pt_input_folder, "checkpoint")
...
for i in range(len(wsi_path_list)):
    ...
    slide_path = wsi_path_list[i]
    slide_id = os.path.basename(slide_path).split('.')[0]
    checkpoint_path = os.path.join(pt_input_folder, "checkpoint", f"{slide_id}.pt")

    if args.save_checkpoints and os.path.exists(checkpoint_path):
        print(f"Checkpoint exists for {slide_id}, loading from disk.")
        embedding_tensor = torch.load(checkpoint_path)
        embeddings.append(embedding_tensor)
        ground_truth.append(torch.tensor(labels[i]))
        continue

    print(f"sample: {i+1}/{len(wsi_path_list)}")
    print(f"path: " + slide_path)

    ## Prepare data for evaluation
    t0_ = time.time()
    all_dataset = WSIDatasetByInstance([slide_path], [tile_coordinate_list[i]], [labels[i]],
    ...
    if args.save_checkpoints:
        torch.save(embeddings_, checkpoint_path)
